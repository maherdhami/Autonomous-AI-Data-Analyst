    ...
    def invoke_chain_with_retry(self, prompt: ChatPromptTemplate, variables: dict, max_retries: int = 2) -> str:
        ...
        for attempt in range(max_retries):
            try:
                ...
                err_str = str(e)
                logger.warning(f"LLM invoke attempt {attempt + 1} failed: {err_str}")
                last_exception = e
                # If invalid API key / authentication error, don't wait and retry repeatedly
                if "401" in err_str or "invalid_api_key" in err_str or "unauthorized" in err_str.lower():
                    break
                time.sleep(1)
                
        raise RuntimeError(f"LLM Service invocation failed: {last_exception}")
